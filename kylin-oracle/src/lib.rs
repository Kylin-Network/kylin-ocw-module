...
use frame_support::{dispatch::DispatchResultWithPostInfo,log, IterableStorageMap, pallet_prelude::*, 
...
use hex::ToHex;
use lite_json::{json::{JsonValue, NumberValue} ,Serialize as JsonSerialize};
use scale_info::TypeInfo;
use sp_std::{convert::TryFrom, borrow::ToOwned, prelude::*, str,vec,vec::Vec};
...
pub mod pallet	{
	...
	#[pallet::config]
	pub trait Config: CreateSignedTransaction<Call<Self>> + frame_system::Config + pallet_balances::Config  where <Self as frame_system::Config>:: AccountId: AsRef<[u8]> + ToHex {
	...
	impl<T: Config> Pallet<T>
		where
			T:: AccountId: AsRef<[u8]> + ToHex, T: pallet_balances::Config
	{


		#[pallet::weight(10_000 + T::DbWeight::get().writes(1))]
		pub fn query_data(origin: OriginFor<T>, para_id: Option<ParaId>, feed_name: Vec<u8>) -> DispatchResult
		{
		...
		#[pallet::weight(10_000 + T::DbWeight::get().writes(1))]
		pub fn write_data_onchain(origin: OriginFor<T>, feed_name: Vec<u8>, data: Vec<u8>) -> DispatchResult
		{
			ensure_signed(origin.clone())?;
			let submitter_account_id = ensure_signed(origin.clone())?;
			let new_feed_name = (str::from_utf8(b"custom_").unwrap().to_owned() + str::from_utf8(&feed_name).unwrap()).as_bytes().to_vec();
			let result = Self::ensure_account_owns_table(submitter_account_id.clone(),new_feed_name.clone());
			match result {
				Ok(()) => {
					Self::add_data_request(Some(submitter_account_id), None, None,new_feed_name, data, false)
				},
				_ => result
			}
		}
		...
		#[pallet::weight(10_000 + T::DbWeight::get().writes(1))]
		pub fn submit_data_signed(origin: OriginFor<T>, block_number: T::BlockNumber, key: u64, data: Vec<u8>) -> DispatchResult {
		...
		#[pallet::weight(10_000 + T::DbWeight::get().writes(1))]
		pub fn submit_data_unsigned(origin: OriginFor<T>, block_number: T::BlockNumber, key: u64, data: Vec<u8>) -> DispatchResult {
		...
		#[pallet::weight(10_000 + T::DbWeight::get().writes(1))]
		pub fn submit_data_via_api(origin: OriginFor<T>, para_id: Option<ParaId>, url: Vec<u8>, feed_name:Vec<u8>) -> DispatchResult
		{
			ensure_signed(origin.clone())?;
			let submitter_account_id = ensure_signed(origin.clone())?;
			let new_feed_name = (str::from_utf8(b"custom_").unwrap().to_owned() + str::from_utf8(&feed_name).unwrap()).as_bytes().to_vec();
			...
			match result {
				Ok(()) => {
					Self::add_data_request(Some(submitter_account_id), para_id, Some(url), new_feed_name,Vec::new(),false)
				},
				_ => result
			}
		}
		...
		#[pallet::weight(10_000 + T::DbWeight::get().writes(1))]
		pub fn xcm_submit_data_via_api(origin: OriginFor<T>, url: Vec<u8>, feed_name:Vec<u8>) -> DispatchResult
		{
			...
			match result {
				Ok(()) => {
					Self::add_data_request(Some(submitter_account_id),Some(requester_para_id), Some(url), new_feed_name,Vec::new(), false)
				},
				_ => result
			}
		}
		...
		pub fn submit_price_feed(origin: OriginFor<T>, para_id: Option<ParaId>, requested_currencies: Vec<u8>) -> DispatchResult
		{
			...
			let result = Self::ensure_account_owns_table(submitter_account_id.clone(),feed_name.clone());
			match result {
				Ok(()) => {
					...
					Self::add_data_request(Some(submitter_account_id), para_id, Some(url.as_bytes().to_vec()), "price_feeding".as_bytes().to_vec(),Vec::new(),false)
				},
				_ => result
			}
		}
		...
		pub fn submit_price_feedd(origin: OriginFor<T>, para_id: Option<ParaId>, requested_currencies: Vec<u8>) -> DispatchResult
		{
			let submitter_account_id = ensure_signed(origin.clone())?;
			let feed_name = "price_feeding".as_bytes().to_vec();
			let result = Self::ensure_account_owns_table(submitter_account_id.clone(),feed_name.clone());
		...
		pub fn clear_processed_requests_unsigned(
			...
			for key in processed_requests.iter(){
				...
				let encoded_hash = hex::encode(sp_runtime::traits::BlakeTwo256::hash(processed_request.clone().encode().as_slice()).as_bytes()).as_bytes().to_vec();
				if processed_request.is_query {
					Self::deposit_event(Event::ReadFromDWH(processed_request.para_id,encoded_hash.clone(), processed_request.feed_name.clone(), processed_request.clone(),processed_request.processed_block_number.clone().unwrap()));
				} else {
					//insert to api
					<ApiQueue<T>>::insert(key,processed_request.clone());
					let feed_owner = Self::feed_account_lookup(processed_request.feed_name.clone()).0;
					<FeedAccountLookup<T>>::insert(processed_request.feed_name.clone(), (&feed_owner, encoded_hash.clone()));
					Self::deposit_event(Event::SavedToDWH(processed_request.para_id,encoded_hash.clone(), processed_request.feed_name.clone(), processed_request.clone(),processed_request.processed_block_number.clone().unwrap()));
				}
	...
	pub enum Event<T: Config> where <T as frame_system::Config>:: AccountId: AsRef<[u8]> + ToHex {
		...
		SavedToDWH(Option<ParaId>, Vec<u8>, Vec<u8>, DataRequest<ParaId, T::BlockNumber, T::AccountId>, T::BlockNumber),
		ReadFromDWH(Option<ParaId>, Vec<u8>, Vec<u8>, DataRequest<ParaId, T::BlockNumber, T::AccountId>, T::BlockNumber),
		...
		QueryFeeAwarded(T::AccountId,<<T as pallet::Config>::Currency as Currency<<T as frame_system::Config>::AccountId>>::Balance, Vec<u8>)
	}

	#[pallet::validate_unsigned]
	impl<T: Config> ValidateUnsigned for Pallet<T>
		where T:: AccountId: AsRef<[u8]> + ToHex, T: pallet_balances::Config {
		type Call = Call<T>;
		...
		fn validate_unsigned(
			...
			if let Call::submit_data_unsigned { block_number,key:_, data:_ } = call {
				Self::validate_transaction(block_number)
			} else if let Call::clear_processed_requests_unsigned { block_number,processed_requests:_ } = call {
				Self::validate_transaction(block_number)
			} else if let Call::clear_api_queue_unsigned { block_number,processed_requests:_ } = call {
				Self::validate_transaction(block_number)
			} else {
	...
	pub type DataRequests<T: Config> = StorageMap<_, Identity, u64, DataRequest<ParaId, T::BlockNumber, T::AccountId>, ValueQuery>;

	#[pallet::storage]
	#[pallet::getter(fn saved_data_requests)]
	pub type SavedRequests<T: Config> = StorageMap<_, Identity, u64, DataRequest<ParaId, T::BlockNumber, T::AccountId>, ValueQuery>;

	#[pallet::storage]
	#[pallet::getter(fn api_queue)]
	pub type ApiQueue<T: Config> = StorageMap<_, Identity, u64, DataRequest<ParaId, T::BlockNumber, T::AccountId>, ValueQuery>;
	...
	pub(super) type FeedAccountLookup<T: Config> = StorageMap<_, Identity, Vec<u8>, (T::AccountId,Vec<u8>), ValueQuery>;
	...
	AccountId: Encode + Default + Encode + Clone + Eq + PartialEq + sp_std::fmt::Debug + AsRef<[u8]> + ToHex ,
{
	...
	fn to_json_string(&self, encoded_value:Vec<u8>) -> Vec<u8>
	{
		...
		object_elements.push((requested_block_number_key, JsonValue::Number(requested_block_number)));

		let processed_block_number_key = str::from_utf8(b"processed_block_number").unwrap().chars().collect();
		let processed_block_number = NumberValue
		{
			integer: self.processed_block_number.clone().unwrap().unique_saturated_into() as i64,
			fraction: 0,
			fraction_length: 0,
			exponent: 0,
		};
		object_elements.push((processed_block_number_key, JsonValue::Number(processed_block_number)));
		...
		object_elements.push((requested_timestamp_key, JsonValue::Number(requested_timestamp)));

		let processed_timestamp_key = str::from_utf8(b"processed_timestamp").unwrap().chars().collect();
		...
		let encoded_hash = hex::encode(sp_runtime::traits::BlakeTwo256::hash(encoded_value.as_slice()).as_bytes()).chars().collect();
...
where
	...
	fn ensure_account_owns_table(submitter_account_id: T::AccountId, feed_name: Vec<u8>) -> DispatchResult {
	...
	fn query_feed(submitter_account_id: T::AccountId, para_id:Option<ParaId>, feed_name: Vec<u8>) -> DispatchResult {
		let feed_exists = FeedAccountLookup::<T>::contains_key(feed_name.clone());
		if feed_exists {
			...
			let total_reward = {
				let call =
					Call::query_data { para_id: para_id.clone(), feed_name: feed_name.clone() };
			...
			Self::add_data_request(Some(submitter_account_id), para_id, Some(url.as_bytes().to_vec()),feed_name.clone(),Vec::new(),true)
		} else {
	...
	fn add_data_request(account_id:Option<T::AccountId>,para_id:Option<ParaId>, url: Option<Vec<u8>>, feed_name:Vec<u8>, payload:Vec<u8>, is_query:bool) -> DispatchResult {
		...
		if !is_query {
		Self::deposit_event(Event::SubmitNewData(para_id, feed_name.clone(), url.clone(), account_id.clone(), block_number));
		}
	...
	fn send_response_to_parachain(block_number: T::BlockNumber, key:u64) -> DispatchResult {
		...
		if saved_request.para_id.is_some(){
			match T::XcmSender::send_xcm(
				(1, Junction::Parachain(saved_request.para_id.unwrap().into())),
				Xcm(vec![Transact {
					origin_type: OriginKind::Native,
					require_weight_at_most: 1_000,
					call: <T as Config>::Call::from(Call::<T>::receive_response_from_parachain { feed_name: saved_request.feed_name.clone(), response: saved_request.payload.clone() }).encode().into(),
				},
				])) {
	...
	fn fetch_data_and_send_signed(block_number: T::BlockNumber) -> Result<(), &'static str> {
		...
		for (key, val) in <DataRequests<T> as IterableStorageMap<_, _>>::iter() {
			...
			let results = signer.send_signed_transaction(|_account| Call::submit_data_signed{ block_number: block_number, key: key, data:response.clone()});
		...
		if processed_requests.iter().count() > 0 {
			let results = signer.send_signed_transaction(|_account| Call::clear_processed_requests_unsigned { block_number: block_number, processed_requests: processed_requests.clone() });
			for (acc, res) in &results {
				match res {
		...
		if queue_to_api.iter().count() > 0 {
			let result = SubmitTransaction::<T, Call<T>>::submit_unsigned_transaction(Call::clear_api_queue_unsigned { block_number: block_number, processed_requests: queue_to_api }.into());
			if let Err(e) = result {
	...
	fn fetch_data_and_send_raw_unsigned(block_number: T::BlockNumber) -> Result<(), &'static str> {
		...
		for (key, val) in <DataRequests<T> as IterableStorageMap<_, _>>::iter() {
			...
			processed_requests.push(key);
			let result = SubmitTransaction::<T, Call<T>>::submit_unsigned_transaction(Call::submit_data_unsigned{ block_number: block_number, key: key, data: response }.into());
		...
		if processed_requests.iter().count() > 0 {
			let result = SubmitTransaction::<T, Call<T>>::submit_unsigned_transaction(Call::clear_processed_requests_unsigned{ block_number: block_number, processed_requests: processed_requests }.into());
	...
	fn submit_http_post_request(url: Vec<u8>, val: DataRequest<ParaId, T::BlockNumber, T::AccountId>) -> Result<Vec<u8>, http::Error> {
		...
		let request = http::Request::post(str::from_utf8(&url).unwrap(),vec![request_body.clone()])
