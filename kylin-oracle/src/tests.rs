...
use xcm::latest::{
    Error as XcmError, Junctions::Here, MultiAsset, MultiLocation, NetworkId, Parent, SendXcm, Xcm,
};
use xcm_builder::{
...
use xcm::opaque::latest::prelude::XcmResult;
...
parameter_types! {
    ...
    //pub RelayChainOrigin: Origin = cumulus_pallet_xcm::Origin::SiblingParachain(ParaId::from(1000)).into();
}
...
fn should_save_data_onchain_for_signed_data_submissions() {
    ...
    let expected_response = br#"{"USD": 50000}"#.to_vec();
    t.execute_with(|| {
        ...
        if let Call::KylinOracle(crate::Call::submit_data_signed(_block_number, _key, response)) =
            tx2.call
        {
...
#[test]
#[ignore = "need to find the way to pass generic origin"]
fn should_save_data_onchain_for_signed_data_submissions_xcm() {
    const PHRASE: &str =
        "news slush supreme milk chapter athlete soap sausage put clutch what kitten";
    let (offchain, offchain_state) = testing::TestOffchainExt::new();
    let (pool, pool_state) = testing::TestTransactionPoolExt::new();
    ...
    let expected_response = br#"{"USD": 50000}"#.to_vec();
    t.execute_with(|| {
        KylinOracle::submit_price_feed_xcm(
            RelayChainOrigin::get(),
            str::from_utf8(b"btc_usd").unwrap().as_bytes().to_vec(),
        )
        .unwrap();
        KylinOracle::fetch_data_and_send_signed(1).unwrap();
...
fn should_save_data_onchain_for_unsigned_submissions() {
    ...
    t.execute_with(|| {
        ...
        if let Call::KylinOracle(crate::Call::submit_data_unsigned(_block_number, _key, response)) =
            tx2.call
        {
            assert_eq!(response, expected_response);
        }
...
#[test]
fn should_add_data_request_for_the_api_for_signed() {
    const PHRASE: &str =
        "news slush supreme milk chapter athlete soap sausage put clutch what kitten";
    let (offchain, offchain_state) = testing::TestOffchainExt::new();
    let (pool, pool_state) = testing::TestTransactionPoolExt::new();
    ...
    t.register_extension(TransactionPoolExt::new(pool));
    t.register_extension(KeystoreExt(Arc::new(keystore)));
    mock_response_via_passed_api_url(
        &mut offchain_state.write(),
        "https://api.kylin-node.co.uk/prices?currency_pairs=kyl_usd",
    );
    let expected_response = br#"{"USD": 10}"#.to_vec();
    t.execute_with(|| {
        KylinOracle::submit_data_via_api(
            Origin::signed(Default::default()),
            None,
            str::from_utf8(b"https://api.kylin-node.co.uk/prices?currency_pairs=kyl_usd")
                .unwrap()
                .as_bytes()
                .to_vec(),
            str::from_utf8(b"test").unwrap().as_bytes().to_vec(),
        )
        .unwrap();
        KylinOracle::fetch_data_and_send_signed(1).unwrap();
    });
...
#[test]
fn should_add_data_request_for_the_api_unsigned() {
    let (offchain, offchain_state) = testing::TestOffchainExt::new();
    let (pool, pool_state) = testing::TestTransactionPoolExt::new();
...
#[test]
fn should_fetch_data_via_http_call_and_get_result() {
    let (offchain, offchain_state) = testing::TestOffchainExt::new();
    ...
    mock_response(&mut offchain_state.write());
    let expected_response = br#"{"USD": 50000}"#.to_vec();
    let urls: Vec<u8> = "https://api.kylin-node.co.uk/prices?currency_pairs=btc_usd"
        .as_bytes()
        .to_vec();
    t.execute_with(|| {
        let response = KylinOracle::fetch_http_get_result(urls).unwrap();
        assert_eq!(response, expected_response);
    });
}
...
fn mock_response_via_passed_api_url(state: &mut testing::OffchainState, url: &str) {
    state.expect_request(testing::PendingRequest {
        method: "GET".into(),
        uri: url.into(),
        response: Some(br#"{"USD": 10}"#.to_vec()),
        sent: true,
        ..Default::default()
    });
}

//TODO need to fix and try to return error
fn mock_response_via_passed_api_url_error(state: &mut testing::OffchainState, url: &str) {
    state.expect_request(testing::PendingRequest {
        method: "GET".into(),
        uri: url.into(),
        response: Some(br#"{"USD": 10}"#.to_vec()),
